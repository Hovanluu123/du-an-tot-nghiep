        ...
        <form id="productForm" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" 
                           ...
                           min="0"
                           step="1000"
                           value="{{ (int) ($product->sale_price ?? 0) }}"
                           class="w-full px-4 py-3 border border-[#e3e3e0] dark:border-[#3E3E3A] rounded-lg focus:ring-2 focus:ring-[#f53003] focus:border-transparent dark:bg-[#0a0a0a] dark:text-[#EDEDEC] transition-colors"
                           placeholder="0">
                    ...
                    @if($product->images && count($product->images))
                        <div class="flex gap-3 flex-wrap">
                            @foreach($product->images as $img)
                                <img src="{{ asset('storage/' . $img) }}" alt="{{ $product->name }}" class="h-16 w-16 rounded-lg object-cover">
                            @endforeach
                        </div>
                    @else
                        <p class="text-sm text-[#706f6c] dark:text-[#A1A09A]">Chưa có hình ảnh</p>
                    @endif
                </div>
            </div>
            ...
            <div>
                <label for="images" class="block text-sm font-medium text-[#1b1b18] dark:text-[#EDEDEC] mb-2">
                    Tải lên hình ảnh mới (thay thế toàn bộ)
                </label>
                <input type="file" 
                       id="images" 
                       name="images[]" 
            ...
            <div>
                ...
                <textarea id="specifications" 
                          ...
                          placeholder='{"màu": "đỏ", "kích thước": "M", "chất liệu": "cotton"}'>{{ $product->specifications ? json_encode($product->specifications, JSON_UNESCAPED_UNICODE) : '' }}</textarea>
